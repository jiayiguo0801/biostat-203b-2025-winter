...
icu_cohort_model <- as.data.frame(icu_cohort_filled) |>
  ...
  print()
icu_cohort_model <- as.h2o(icu_cohort_model)
h2o.describe(icu_cohort_model)
```
...
if (file.exists("logit_fit.rds")) {
  ...
  message("file exists, no need to train again")
} else {
  message("can't find logit_fit.rds，train logit_fit model.")
  ...
  message("logit_fit training finished and save as logit_fit.rds。")
}
```
...
# Check if rf_fit.rds exists
if (file.exists("rf_fit.rds")) {
  # Load the model if the file exists
  rf_fit <- readRDS("rf_fit.rds")
  message("✅ Loaded rf_fit.rds. Skipping model training.")
} else {
  # Train the model if the file does not exist
  message("⚠️ rf_fit.rds not found. Starting model training...")

  rf_fit <- rf_wf |> 
    tune_grid(
      resamples = folds,  # Ensure the resampling object matches other models
      grid = param_grid,
      metrics = metric_set(roc_auc, accuracy),
      control = control_stack_grid()
    )
  ...
  message("✅ rf_fit training completed and saved as rf_fit.rds.")
}
...
### Fit cross-validation.
```{r}
# Check if gb_fit.rds exists
if (file.exists("gb_fit.rds")) {
  # Load the model if the file exists
  gb_fit <- readRDS("gb_fit.rds")
  message("✅ Loaded gb_fit.rds. Skipping model training.")
} else {
  # Train the model if the file does not exist
  message("⚠️ gb_fit.rds not found. Starting model training...")
  ...
  message("✅ gb_fit training completed and saved as gb_fit.rds.")
}
...
```{r}
library(parallel)
cl <- makeCluster(4) # 限制最大使用 4 核
registerDoParallel(cl)
# Step 1: 生成 mimic_model_st 并 blend_predictions()
if (file.exists("mimic_model_step1.rds")) {
  # 如果中间文件已存在，则直接加载
  mimic_model_st <- readRDS("mimic_model_step1.rds")
  message("mimic_model_step1.rds esists")
} else {
  # 如果文件不存在，则重新创建 stacks() 模型
  message("can't find mimic_model_step1.rds，start stacking model fit")
  
  mimic_model_st <- stacks() %>%
    add_candidates(logit_fit) %>%
    add_candidates(rf_fit) %>%
    add_candidates(gb_fit) %>%
    blend_predictions(penalty = 10^(-6:2), metrics = metric_set(roc_auc, accuracy))
  
  # 保存中间模型
  saveRDS(mimic_model_st, "mimic_model_step1.rds")
  message("mimic_model_step1 save as mimic_model_step1.rds。")
}

# Step 2: 进行 fit_members()，生成最终模型
if (file.exists("mimic_model_final.rds")) {
  # 如果最终模型已存在，则直接加载
  mimic_model_st <- readRDS("mimic_model_final.rds")
  message("mimic_model_final.rds exists")
} else {
  # 如果文件不存在，则继续执行 fit_members()
  message("can't find mimic_model_final.rds")
  
  mimic_model_st <- mimic_model_st %>%
    fit_members()
  
  # 保存最终模型
  saveRDS(mimic_model_st, "mimic_model_final.rds")
  message("mimic_model_final save as mimic_model_final.rds。")
}

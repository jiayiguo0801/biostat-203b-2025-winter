...
ggplot() +
  ...
  geom_point(
    ...
    aes(x = chartdate, y = "Procedure", shape = factor(shape), color = procedure_type),
    size = 3
  ) +
...
# 将时间转换为 POSIXct 类型并排序
vitals_data <- vitals_data |>
  mutate(charttime = as.POSIXct(charttime)) |>
  arrange(subject_id, charttime)

# 创建每六小时的时间窗口并筛选每个窗口的第一条记录
vitals_data_filtered <- vitals_data |>
  mutate(time_window = as.POSIXct(floor(as.numeric(charttime) / (6 * 3600)) * (6 * 3600), origin = "1970-01-01")) |>
...
# 生成折线图
ggplot(vitals_data_filtered, aes(x = charttime, y = value, color = label, group = label)) + 
  geom_line() +  # 画线，强制按时间顺序连接
  geom_point() +  # 在每个数据点上添加点
  facet_wrap(~ label, scales = "free_y") +  # 按照生命体征分面，y轴自适应
  labs(
...
# 1. 划分住院阶段：根据 intime 和 outtime 为数据添加 stage_id 标识
head(vitals_data)
vitals_data <- vitals_data |>
  left_join(ADT10063848, by = "subject_id") |>
  mutate(charttime = as.POSIXct(charttime)) |>
  arrange(subject_id, charttime) |>
  mutate(
    stage_id = paste0("Stage ", dense_rank(intime_data.x))  # 每次住院阶段的唯一标识
  )

# 2. 为每个阶段生成单独的折线图
stages <- unique(vitals_data$stage_id)  # 获取所有的住院阶段

# 3. 循环处理每个阶段的数据，并生成图
plots <- lapply(stages, function(stage) {
  
  # 选择特定阶段的数据
  stage_data <- vitals_data |>
    filter(stage_id == stage) |>
    filter(label %in% c("Heart Rate", "Non Invasive Blood Pressure systolic", 
                        "Non Invasive Blood Pressure diastolic", "Respiratory Rate", "Body Temperature")) |>
    mutate(time_window = as.POSIXct(floor(as.numeric(charttime) / (6 * 3600)) * (6 * 3600), origin = "1970-01-01")) |>
    group_by(subject_id, time_window, label) |>
    slice_min(charttime) |>
    ungroup() |>
  ...
  # 生成折线图
  ggplot(stage_data, aes(x = charttime, y = value, color = label, group = label)) + 
    geom_line() +  # 画线
    geom_point() +  # 添加点
    facet_wrap(~ label, scales = "free_y") +  # 按生命体征分面
    labs(
      title = paste("Vitals of Patient 10063848 during", stage),
      x = "Time",
      y = "Vital Value",
      color = "Vital Type"
    ) +
...
ggplot(time_admissions, aes(x = admission_hour)) +
  ...
  labs(title = "Admission Hour Distribution", x = "Hour of Admission", y = "Frequency")
ggplot(time_admissions, aes(x = admission_minute)) +
  geom_histogram(binwidth = 1, fill = "blue", color = "black") +
  labs(title = "Admission Minute Distribution", x = "Minute of Admission", y = "Frequency")
ggplot(time_admissions, aes(x = length_of_stay)) +
  geom_histogram(binwidth = 1, fill = "blue", color = "black") +
  labs(title = "Length of Hospital Stay", x = "Length of Stay (hours)", y = "Frequency")
...
# Assuming mimic_icu_cohort has columns: los, race, insurance, marital_status, gender, and age_at_intime
...
# Assume mimic_icu_cohort includes LOS and the last available lab measurements
# For example, mimic_icu_cohort might have columns: los, Bicarbonate, Chloride, Creatinine, Glucose, Potassium, Sodium, Hematocrit, wbc

# 1. Calculate correlation coefficients between los and each lab measurement
lab_vars <- c("Bicarbonate", "Chloride", "Creatinine", "Glucose", "Potassium", "Sodium", "Hematocrit", "wbc")

correlations <- mimic_icu_cohort %>%
  select(los, all_of(lab_vars)) %>%
  summarise(across(all_of(lab_vars), ~ cor(los, .x, use = "complete.obs"))) %>%
  pivot_longer(everything(), names_to = "Variable", values_to = "Correlation")

print(correlations)

# 2. Create scatter plots with a linear regression line for each lab variable vs. los
...
# Assume mimic_icu_cohort has columns: los, HR, Systolic_BP, Diastolic_BP, BT, RR

# 1. Calculate correlation coefficients between los and each vital measurement
vital_vars <- c("HR", "NBPs", "NBPd", "BT", "RR")

vital_correlations <- mimic_icu_cohort %>%
  select(los, all_of(vital_vars)) %>%
  summarise(across(all_of(vital_vars), ~ cor(los, .x, use = "complete.obs"))) %>%
  pivot_longer(cols = everything(), names_to = "Variable", values_to = "Correlation")

print(vital_correlations)

# 2. Create scatter plots with a linear regression line for each vital variable vs. los
...
# Assume mimic_icu_cohort has columns:
# - los: Length of ICU stay
# - first_careunit: The first ICU unit (e.g., MICU, SICU, etc.)

# 1. Numerical Summary: Compute group-level summaries for each ICU unit
...
# 2. Graphical Summary: Create a boxplot of ICU LOS by first ICU unit

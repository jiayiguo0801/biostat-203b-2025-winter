...
# Join labitems
# 应该在合并之前先转成长格式！！
labevents <- open_dataset("~/mimic/hosp/labevents.parquet") %>%
  filter(itemid %in% lab_itemids) %>%
  select(subject_id, itemid, valuenum, storetime, hadm_id) %>%
  mutate(itemid = as.integer(itemid)) %>%
...
# Get wider format
# 
labevents_wide <- labevents %>%
  group_by(subject_id, hadm_id, storetime) %>%
  pivot_wider(
    names_from = label,
  ...
  select(subject_id, storetime, Bicarbonate, Chloride, Creatinine, Glucose,
         Potassium, Sodium, Hematocrit, wbc)
print(labevents_wide)

# Join icustays
labevents_tble <- icustays %>%
  left_join(labevents_wide, by = "subject_id") %>%
...
# 1. 读取 ICU 住院数据，并保留必要变量
icustays_tble <- read_csv("~/mimic/icu/icustays.csv.gz") %>%
  select(subject_id, stay_id, intime, outtime) %>%
  mutate(
    subject_id = as.numeric(subject_id),
    stay_id = as.integer(stay_id),
    intime = as.POSIXct(intime),   # 假设数据中 intime 格式正确
    outtime = as.POSIXct(outtime)
  )
print(icustays_tble)

# 2. 读取 d_items 数据，并筛选出感兴趣的 itemid，同时转换 itemid 为 numeric
...
# 3. 读取 chartevents 数据（Parquet 格式），筛选感兴趣的 itemid，
#    并将 itemid 与 subject_id 转换为 numeric
chartevents_raw <- open_dataset("~/mimic/icu/chartevents.parquet") %>%
  filter(itemid %in% c(220045, 220179, 220180, 223761, 220210)) %>%
  mutate(
...
# 由于 Arrow 后端不支持 as.POSIXct()，在 collect() 后转换 charttime


# 4. 限定 chartevents 数据：仅保留 icustays_tble 中的患者，
#    并加入 ICU 时间信息（intime, outtime）以限制在 ICU 内的记录，
#    再针对每个 ICU stay 和每个项目取 ICU 内首次测量记录
chartevents_tble <- chartevents_raw %>%
  # 仅保留 icustays 中的患者
  semi_join(icustays_tble, by = "subject_id") %>%
  # 加入 ICU 住院时间信息（这里通过 subject_id 连接，注意同一患者可能有多次 ICU 住院）
  left_join(select(icustays_tble, subject_id, stay_id, intime, outtime),
            by = "subject_id") %>%
  # 保留 ICU 期间的记录
  filter(charttime >= intime, charttime <= outtime) %>%
  # 针对每个 ICU 住院（stay_id）和每个测量项目（itemid），按 charttime 排序后保留第一条记录
  group_by(stay_id, itemid) %>%
  arrange(charttime, .by_group = TRUE) %>%
  slice_head(n = 1) %>%
  ungroup() %>%
  # 加入 d_items_tble 中的 label 信息
  left_join(select(d_items_tble, itemid, label), by = "itemid") %>%
  # 保留关键变量
  select(stay_id, subject_id, label, valuenum)
  
# 5. 转换为宽格式：每个 ICU 住院一行，列为各 vital 测量值
chartevents_tble <- chartevents_tble %>%
  pivot_wider(names_from = label, values_from = valuenum)

# 6. 重命名变量：确保与题目要求一致
# 假定 d_items_tble 中的 label 分别为：
#   "Heart Rate" 
